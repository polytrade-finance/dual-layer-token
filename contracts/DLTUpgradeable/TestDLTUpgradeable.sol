...
contract TestDLTUpgradeable is Initializable, DLTUpgradeable, DLTEnumerableUpgradeable, DLTPermitUpgradeable {
    ...
    function initDLTUnchained(string memory name, string memory symbol) external {
    ...
    function initDLTPermit(string memory name, string memory version) external {
        ...
        function initDLTPermitUnchained(string memory name, string memory version) external {
        __DLTPermit_init_unchained(name, version);
    }

    // solhint-disable-next-line ordering
    function mint(address account, uint256 mainId, uint256 subId, uint256 amount) external {
        _safeMint(account, mainId, subId, amount);
    }

    function burn(address account, uint256 mainId, uint256 subId, uint256 amount) external {
        _burn(account, mainId, subId, amount);
    }

    function transfer(address sender, address recipient, uint256 mainId, uint256 subId, uint256 amount) external {
        _transfer(sender, recipient, mainId, subId, amount);
    }

    function allow(address sender, address recipient, uint256 mainId, uint256 subId, uint256 amount) external {
    ...
    function _mint(address recipient, uint256 mainId, uint256 subId, uint256 amount)
        internal
        virtual
        override(DLTUpgradeable, DLTEnumerableUpgradeable)
    {
